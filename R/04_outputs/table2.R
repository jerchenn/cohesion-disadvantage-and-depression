...
# Recomputes primary + washout under the PRIMARY (factor) specification so the numbers match
...
mW <- coxph(f, cox[cox$time_days>180,])  # 180-day washout
...
cat("Footnote: HR per 1-SD cohesion. Reweighted row uses the parsimonious specification;",
    "\nprimary and washout use the fully adjusted specification.\n")
